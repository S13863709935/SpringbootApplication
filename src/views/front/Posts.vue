<template>
  <div style="margin: 10px auto; width: 50%">
    ...
    <div style="display: flex; flex-wrap: wrap; margin-bottom: 10px">
      <div class="card circle" @click="loadCircle('All')" :class="{'circle-active' : circle === 'All' }">
...
export default {
  ...
  methods: {
    loadCircle(circle) {  // 分页Search
      this.circle = circle
      this.load(1)
    },
    load(pageNum) {  // 分页Search
      if (pageNum) this.pageNum = pageNum
      this.$request.get('/posts/selectFrontPage', {
        params: {
          pageNum: this.pageNum,
          pageSize: this.pageSize,
          circle: this.circle === 'All' ? null : this.circle,
        }
      }).then(res => {
        if (res.code === '200') {
