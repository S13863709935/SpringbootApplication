<template>
  <div>
    ...
    <el-dialog title="Post Information" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="120px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="Community" prop="circle">
          <el-input v-model="form.circle" placeholder="Community"></el-input>
        </el-form-item>
...
<script>
export default {
  name: "Posts", // Corrected from 'title' to standard Vue property 'name'
  data() {
    return {
      tableData: [],  // All data
  ...
  methods: {
    changeStatus(row, status) {
      // The Chinese status is maintained for backend logic, but we log it as an action conceptually.
      this.$confirm('Are you sure you want to change status to ' + (status === '通过' ? 'Approved' : 'Rejected') + '?', 'Confirm Action', {type: "warning"}).then(response => {
